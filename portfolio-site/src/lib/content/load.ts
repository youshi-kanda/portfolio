...
import { site, type SiteContent } from './site.ts';
...
export interface ContentBundle {
  ...
  uiCopy: UiCopyItem[];
  /**
   * The singleton section content. It is a JSON module rather than a
   * collection, and until now that also kept it outside the bundle the gates
   * run on — so the gates could not see the workflow steps, the stack rows or
   * the principles at all. Being one record is a loading decision; it is not a
   * reason for the copy in it to be ungoverned.
   */
  site: SiteContent;
}
